    ...
    <div className="space-y-3">
      {suggestions.map(sug => (
        <div key={sug.id} className="bg-[var(--card)] border border-[var(--border)] rounded-xl p-4 flex flex-col md:flex-row gap-4 items-start">
          <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${priorityStyles[sug.priority]}`}>
          ...
          <span className={`text-xs px-2 py-1 rounded font-medium ${priorityStyles[sug.priority]}`}>
